...
@commands.guild_only()
@commands.group(name="refund")
async def refund(self, ctx: commands.Context):
    """Create a reaction ticket system in your server"""
    pass


@checks.admin()
@refund.group(invoke_without_command=True, aliases=["set"])
async def settings(self, ctx):
    """Manage settings for refund"""
    await ctx.send_help()
    guild_settings = await self.config.guild(ctx.guild).all()
    channel_id, message_id = list(map(int, guild_settings["msg"].split("-")))

    ticket_channel = getattr(
        self.bot.get_channel(channel_id), "name", "Not set")
    ticket_category = getattr(self.bot.get_channel(
        guild_settings["category"]), "name", "Not set")
    archive_category = getattr(
        self.bot.get_channel(
            guild_settings["archive"]["category"]), "name", "Not set"
    )
    report_channel = getattr(self.bot.get_channel(
        guild_settings["report"]), "name", "Not set")
