...
class Suggestion(commands.Cog):
    ...
    async def suggest(self, ctx: commands.Context, *, suggestion: str):
        ...
        embed.set_author(
            name=f"Предложение от {ctx.author.display_name}",
            icon_url=ctx.author.avatar_url,
        )
        embed.set_footer(
            text=f"Предложение от {ctx.author.name}#{ctx.author.discriminator} ({ctx.author.id})"
        )
        ...
        try:
            await ctx.author.send(
                content="Ваше предложение было отправлено!", embed=embed
            )
    ...
    async def reject(
        ...
        *,
        ...
        """Отклонить предложение. Причина не обязательная"""
    ...
    async def suggestset_settings(self, ctx: commands.Context):
        ...
        embed.add_field(name="Same channel*:",
                        value=str(data["same"]), inline=False)
    ...
    async def suggestset_globalset_settings(self, ctx: commands.Context):
        ...
        embed.add_field(name="Ignored servers:",
                        value=servers_text, inline=False)
    ...
    async def _build_suggestion(
        ...
        embed.set_footer(
            text=f"Предложение от {op_name}#{op_discriminator} ({op_id})")
