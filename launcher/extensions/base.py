...
class launcherBaseMixin(MixinMeta):
    ...
    async def lock_ticket(self, ctx, ticket):
        ...
        await channel.send(
            (
                ...
                f"`{ctx.prefix}launcher unlock {channel.mention}`, or through the queue."
            )
        )
    ...
    def is_support_or_superior():
        async def predicate(ctx):
            guild_settings = await ctx.bot.get_cog("launcher").config.guild(ctx.guild).all()
    ...
    @launcher.command(aliases=["unlock"])
    async def lock(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        Note: You must have a launcher Support Role or a Red Admin Role to run this."""
        if channel is None:
            channel = ctx.channel
    ...
    @is_support_or_superior()
    @launcher.command(aliases=["moderator", "mod"])
    async def assign(
        ...
        if not (
            ...
            await ctx.send(
                ...
                "or have a launcher Support Role."
            )
            return
    ...
    @launcher.command(aliases=["tickets"])
