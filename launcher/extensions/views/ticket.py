...
from launcher.extensions.views.confirmation import Confirmation
from launcher.extensions.views.alert import Alert

if TYPE_CHECKING:
    from launcher.extensions.views.queue import Queue
...
class Ticket(discord.ui.View):
    ...
    async def build_embed(self):
        ...
        user = getattr(self.ctx.guild.get_member(
            self.ticket["user"]), "mention", "Removed user")
        self.embed.add_field(name="Ticket Author", value=user, inline=True)

        added_users = []
        for user in self.ticket["added"]:
            added_users.append(
                getattr(self.ctx.guild.get_member(user), "mention", "Removed user"))
        self.embed.add_field(
            name="Ticket Members",
            value=humanize_list(
                added_users) or "This ticket does not have any members",
            inline=True,
        )
        ...
        if self.ticket["assigned"]:
            moderator = getattr(
                self.ctx.guild.get_member(
                    self.ticket["assigned"]), "mention", "Removed user"
            )
        self.embed.add_field(name="Ticket Moderator",
                             value=moderator, inline=True)

        timestamp = datetime.fromtimestamp(self.ticket["opened"]).strftime(
            "%B %d, %Y at %H:%M UTC"
        )
        self.embed.add_field(name="Ticket Creation Date",
    ...
    async def assign_moderator(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        prompt = discord.Embed(
            ...
            description=(
                ...
                "assign to this ticket. They must be a Red Admin or have a launcher "
        ...
        if not (
            ...
            alert = Alert(
                title="Failed to Assign Moderator",
                description="The specified user is not a Red Admin or launcher Support",
    ...
    async def close_ticket(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if result:
            ...
            archive = self.ctx.bot.get_channel(
                guild_settings["archive"]["category"])
            added_users = [
