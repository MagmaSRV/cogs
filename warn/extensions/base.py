...
class WarnBaseMixin(MixinMeta):
    async def report_close(self, ctx, ticket, author, guild_settings, reason):
        ...
        if guild_settings["report"] != 0:
            ...
            if reporting_channel:
                if await self.embed_warned(reporting_channel):
                    embed = discord.Embed(
                        title="Ticket Closed",
                        description=(
    ...
    @RTMixin.warn.command(name="remove")
    ...
    @RTMixin.warn.command(name="name")
    ...
    async def lock_ticket(self, ctx, ticket):
        ...
        await channel.send(
            (
                ...
                f"`{ctx.prefix}warn unlock {channel.mention}`, or through the queue."
            )
        )
    ...
    def is_support_or_superior():
        async def predicate(ctx):
            guild_settings = await ctx.bot.get_cog("Warn").config.guild(ctx.guild).all()
    ...
    @RTMixin.warn.command(aliases=["unlock"])
    async def lock(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        Note: You must have a Warn Support Role or a Red Admin Role to run this."""
        if channel is None:
            channel = ctx.channel
    ...
    @is_support_or_superior()
    @RTMixin.warn.command(aliases=["moderator", "mod"])
    async def assign(
    ...
    @RTMixin.warn.command(aliases=["tickets"])
