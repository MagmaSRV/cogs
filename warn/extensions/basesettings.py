...
from warn.extensions.abc import MixinMeta
from warn.extensions.mixin import RTMixin


class WarnBaseSettingsMixin(MixinMeta):
        ...
        if user:
            async with self.config.guild(ctx.guild).blacklist() as blacklist:
                if user.id in blacklist:
                    ...
                    await ctx.send(
                        f"{user.display_name} has been removed from the warn blacklist."
                    )
                else:
                    blacklist.append(user.id)
                    await ctx.send(
                        f"{user.display_name} has been add to the warn blacklist."
                    )
        else:
    ...
    async def maxtickets(self, ctx, number: int, send_dm: Optional[bool] = None):
        ...
        Examples (in situation where these commands are run one after another):
            `[p]warn settings precreationsettings maxtickets 1 False`
            Users can make 1 ticket, no DM.

            `[p]warn settings precreationsettings maxtickets 2 True`
            Users can make 2 tickets, send DM.

            `[p]warn settings precreationsettings maxtickets 1`
    ...
    async def ticket_names_list(self, ctx):
        ...
        embed.set_footer(
            text=f"Use {ctx.prefix}warn settings postcreationsettings "
    ...
    async def ticket_names_add(self, ctx, *, name: str):
        ...
        if pred.result is True:
            ...
            await ctx.send(
                ...
                f"`{ctx.prefix}warn settings postcreationsettings "
    ...
    async def ticket_names_select(self, ctx, index: int):
        ...
        `[p]warn settings postcreationsettings ticketnames list`"""
        real_index = index - 1
        settings = await self.config.guild(ctx.guild).presetname()
        if settings["chosen"] == real_index:
        ...
        if index > len(settings["presets"]):
            await ctx.send(
                ...
                f"command `{ctx.prefix}warn settings postcreationsettings ticketnames list`."
            )
            return
    ...
    async def enable(self, ctx, yes_or_no: Optional[bool] = None):
        ...
        if not ctx.channel.permissions_for(ctx.guild.me).administrator:
            await ctx.send(
                "I require Administrator permission to start the Warn system.  Note that "
        ...
        if channel_id == message_id == 0:
            await ctx.send(
                ...
                f"`{ctx.prefix}warn settings precreationsettings setmsg`."
            )
            return
        ...
        if not category_id:
            await ctx.send(
                ...
                f"`{ctx.prefix}warn settings postcreationsettings category`."
            )
            return
        ...
        if archive["enabled"]:
            if not archive["category"]:
                await ctx.send(
                    ...
                    f"`{ctx.prefix}warn settings closesettings archive category`."
                )
                return
        ...
        if report != 0:
            ...
            if not report_channel:
                await ctx.send(
                    ...
                    f"`{ctx.prefix}warn settings closesettings reports`."
                )
