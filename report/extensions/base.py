...
class reportBaseMixin(MixinMeta):
    ...
    async def lock_ticket(self, ctx, ticket):
        ...
        await channel.send(
            (
                ...
                f"`{ctx.prefix}report unlock {channel.mention}`, or through the queue."
            )
        )
    ...
    def is_support_or_superior():
        async def predicate(ctx):
            guild_settings = await ctx.bot.get_cog("report").config.guild(ctx.guild).all()
    ...
    async def lock(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        Note: You must have a report Support Role or a Red Admin Role to run this."""
        if channel is None:
            channel = ctx.channel
    ...
    @is_support_or_superior()
    @report.command(aliases=["moderator", "mod"])
    async def assign(
    ...
    @report.command(aliases=["tickets"])
