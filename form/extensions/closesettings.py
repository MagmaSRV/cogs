...
class formCloseSettingsMixin(MixinMeta):
    @settings.group()
    ...
    async def forms(self, ctx, channel: discord.TextChannel = None):
        """Set a channel to make a mini form in when a ticket is closed or opened.

        If left blank, this will disable forms."""
        saving = getattr(channel, "id", 0)
        await self.config.guild(ctx.guild).form.set(saving)

        if not channel:
            await ctx.send("forming has been disabled.")
        else:
            await ctx.send(f"forming channel has been set to {channel.mention}")
