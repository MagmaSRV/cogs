...
from form.extensions.abc import MixinMeta
from form.extensions.mixin import settings


class formBaseSettingsMixin(MixinMeta):
        ...
        if user:
            async with self.config.guild(ctx.guild).blacklist() as blacklist:
                if user.id in blacklist:
                    ...
                    await ctx.send(
                        f"{user.display_name} has been removed from the form blacklist."
                    )
                else:
                    blacklist.append(user.id)
                    await ctx.send(
                        f"{user.display_name} has been add to the form blacklist."
                    )
        else:
    ...
    async def maxtickets(self, ctx, number: int, send_dm: Optional[bool] = None):
        ...
        Examples (in situation where these commands are run one after another):
            `[p]form settings precreationsettings maxtickets 1 False`
            Users can make 1 ticket, no DM.

            `[p]form settings precreationsettings maxtickets 2 True`
            Users can make 2 tickets, send DM.

            `[p]form settings precreationsettings maxtickets 1`
    ...
    async def ticket_names_list(self, ctx):
        ...
        embed.set_footer(
            text=f"Use {ctx.prefix}form settings postcreationsettings "
    ...
    async def ticket_names_add(self, ctx, *, name: str):
        ...
        if pred.result is True:
            ...
            await ctx.send(
                ...
                f"`{ctx.prefix}form settings postcreationsettings "
    ...
    async def ticket_names_select(self, ctx, index: int):
        ...
        `[p]form settings postcreationsettings ticketnames list`"""
        real_index = index - 1
        settings = await self.config.guild(ctx.guild).presetname()
        if settings["chosen"] == real_index:
        ...
        if index > len(settings["presets"]):
            await ctx.send(
                ...
                f"command `{ctx.prefix}form settings postcreationsettings ticketnames list`."
            )
            return
    ...
    async def enable(self, ctx, yes_or_no: Optional[bool] = None):
        ...
        if not ctx.channel.permissions_for(ctx.guild.me).administrator:
            await ctx.send(
                "I require Administrator permission to start the form system.  Note that "
        ...
        if channel_id == message_id == 0:
            await ctx.send(
                ...
                f"`{ctx.prefix}form settings precreationsettings setmsg`."
            )
            return
        ...
        if not category_id:
            await ctx.send(
                ...
                f"`{ctx.prefix}form settings postcreationsettings category`."
            )
            return
        ...
        if archive["enabled"]:
            if not archive["category"]:
                await ctx.send(
                    ...
                    f"`{ctx.prefix}form settings closesettings archive category`."
                )
                return
        ...
        # 5 - forming channel (also if enabled)
        form = await self.config.guild(ctx.guild).form()
        if form != 0:
            form_channel = self.bot.get_channel(form)
            if not form_channel:
                await ctx.send(
                    "forming is enabled but the channel has been deleted. Please reset it with "
                    f"`{ctx.prefix}form settings closesettings forms`."
                )

            if not form_channel.permissions_for(ctx.guild.me).send_messages:
                await ctx.send(
                    "forming is enabled but I do not have proper permissions.  "
