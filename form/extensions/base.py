...
from form.extensions.mixin import form

if discord.__version__ == "2.0.0a" or TYPE_CHECKING:
    from form.extensions.views.queue import Queue


class formBaseMixin(MixinMeta):
    async def form_close(self, ctx, ticket, author, guild_settings, reason):
    ...
    async def ticket_add(self, ctx, user: discord.Member):
        ...
        channel = self.bot.get_channel(
            guild_settings["created"][str(author_id)][index]["channel"])
    ...
    async def lock_ticket(self, ctx, ticket):
        ...
        await channel.send(
            (
                ...
                f"`{ctx.prefix}form unlock {channel.mention}`, or through the queue."
            )
        )
    ...
    def is_support_or_superior():
        async def predicate(ctx):
            guild_settings = await ctx.bot.get_cog("form").config.guild(ctx.guild).all()
    ...
    async def lock(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        Note: You must have a form Support Role or a Red Admin Role to run this."""
        if channel is None:
            channel = ctx.channel
    ...
    @is_support_or_superior()
    @form.command(aliases=["moderator", "mod"])
    async def assign(
