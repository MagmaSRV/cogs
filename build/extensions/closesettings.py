...
class buildCloseSettingsMixin(MixinMeta):
    @settings.group()
    ...
    async def builds(self, ctx, channel: discord.TextChannel = None):
        """Set a channel to make a mini build in when a ticket is closed or opened.

        If left blank, this will disable builds."""
        saving = getattr(channel, "id", 0)
        await self.config.guild(ctx.guild).build.set(saving)

        if not channel:
            await ctx.send("building has been disabled.")
        else:
            await ctx.send(f"building channel has been set to {channel.mention}")
