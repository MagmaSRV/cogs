...
from build.extensions.mixin import build

if discord.__version__ == "2.0.0a" or TYPE_CHECKING:
    from build.extensions.views.queue import Queue


class buildBaseMixin(MixinMeta):
    async def build_close(self, ctx, ticket, author, guild_settings, reason):
    ...
    async def lock_ticket(self, ctx, ticket):
        ...
        await channel.send(
            (
                ...
                f"`{ctx.prefix}build unlock {channel.mention}`, or through the queue."
            )
        )
    ...
    def is_support_or_superior():
        async def predicate(ctx):
            guild_settings = await ctx.bot.get_cog("build").config.guild(ctx.guild).all()
    ...
    async def lock(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        Note: You must have a build Support Role or a Red Admin Role to run this."""
        if channel is None:
            channel = ctx.channel
    ...
    @is_support_or_superior()
    @build.command(aliases=["moderator", "mod"])
    async def assign(
