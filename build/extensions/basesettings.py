...
class buildBaseSettingsMixin(MixinMeta):
        ...
        if user:
            async with self.config.guild(ctx.guild).blacklist() as blacklist:
                if user.id in blacklist:
                    ...
                    await ctx.send(
                        f"{user.display_name} has been removed from the build blacklist."
                    )
                else:
                    blacklist.append(user.id)
                    await ctx.send(
                        f"{user.display_name} has been add to the build blacklist."
                    )
        else:
    ...
    async def maxtickets(self, ctx, number: int, send_dm: Optional[bool] = None):
        ...
        Examples (in situation where these commands are run one after another):
            `[p]build settings precreationsettings maxtickets 1 False`
            Users can make 1 ticket, no DM.

            `[p]build settings precreationsettings maxtickets 2 True`
            Users can make 2 tickets, send DM.

            `[p]build settings precreationsettings maxtickets 1`
    ...
    async def ticket_names_list(self, ctx):
        ...
        embed.set_footer(
            text=f"Use {ctx.prefix}build settings postcreationsettings "
    ...
    async def ticket_names_add(self, ctx, *, name: str):
        ...
        if pred.result is True:
            ...
            await ctx.send(
                ...
                f"`{ctx.prefix}build settings postcreationsettings "
    ...
    async def ticket_names_select(self, ctx, index: int):
        ...
        `[p]build settings postcreationsettings ticketnames list`"""
        real_index = index - 1
        settings = await self.config.guild(ctx.guild).presetname()
        if settings["chosen"] == real_index:
        ...
        if index > len(settings["presets"]):
            await ctx.send(
                ...
                f"command `{ctx.prefix}build settings postcreationsettings ticketnames list`."
            )
            return
    ...
    async def enable(self, ctx, yes_or_no: Optional[bool] = None):
        ...
        if not ctx.channel.permissions_for(ctx.guild.me).administrator:
            await ctx.send(
                "I require Administrator permission to start the build system.  Note that "
        ...
        if channel_id == message_id == 0:
            await ctx.send(
                ...
                f"`{ctx.prefix}build settings precreationsettings setmsg`."
            )
            return
        ...
        if not category_id:
            await ctx.send(
                ...
                f"`{ctx.prefix}build settings postcreationsettings category`."
            )
            return
        ...
        if archive["enabled"]:
            if not archive["category"]:
                await ctx.send(
                    ...
                    f"`{ctx.prefix}build settings closesettings archive category`."
                )
                return
        ...
        # 5 - building channel (also if enabled)
        build = await self.config.guild(ctx.guild).build()
        if build != 0:
            build_channel = self.bot.get_channel(build)
            if not build_channel:
                await ctx.send(
                    "building is enabled but the channel has been deleted. Please reset it with "
                    f"`{ctx.prefix}build settings closesettings builds`."
                )

            if not build_channel.permissions_for(ctx.guild.me).send_messages:
                await ctx.send(
                    "building is enabled but I do not have proper permissions.  "
